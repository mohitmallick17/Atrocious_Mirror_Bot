...
from bot import GDRIVE_FOLDER_ID, IS_TEAM_DRIVE, INDEX_URL, USE_SERVICE_ACCOUNTS, VIEW_LINK, \
...
class GoogleDriveHelper:
    ...
    def __upload_file(self, file_path, file_name, mime_type, GDRIVE_FOLDER_ID):
            ...
            try:
                ...
                if err.resp.get('content-type', '').startswith('application/json'):
                    ...
                    if USE_SERVICE_ACCOUNTS:
                        ...
                        return self.__upload_file(file_path, file_name, mime_type, GDRIVE_FOLDER_ID)
                    else:
    ...
    def upload(self, file_name: str):
        ...
        try:
            if ospath.isfile(file_path):
                ...
                link = self.__upload_file(file_path, file_name, mime_type, GDRIVE_FOLDER_ID)
                if self.__is_cancelled:
                    return
                if link is None:
                ...
                mime_type = 'Folder'
                dir_id = self.__create_directory(ospath.basename(ospath.abspath(file_name)), GDRIVE_FOLDER_ID)
    ...
    def clone(self, link):
        ...
        try:
            ...
            if mime_type == self.__G_DRIVE_DIR_MIME_TYPE:
                dir_id = self.__create_directory(meta.get('name'), GDRIVE_FOLDER_ID)
                ...
                file = self.__copyFile(meta.get('id'), GDRIVE_FOLDER_ID)
    ...
    def __cloneFolder(self, name, local_path, folder_id, GDRIVE_FOLDER_ID):
        ...
        for file in files:
            if file.get('mimeType') == self.__G_DRIVE_DIR_MIME_TYPE:
                ...
                current_dir_id = self.__create_directory(file.get('name'), GDRIVE_FOLDER_ID)
                ...
                self.__copyFile(file.get('id'), GDRIVE_FOLDER_ID)
            if self.__is_cancelled:
                break

    @retry(wait=wait_exponential(multiplier=2, min=3, max=6), stop=stop_after_attempt(3),
           retry=retry_if_exception_type(GCError))
    def __create_directory(self, directory_name, GDRIVE_FOLDER_ID):
        file_metadata = {
            "name": directory_name,
            "description": "Uploaded by Helios-mirror-bot",
    ...
    def __upload_dir(self, input_directory, GDRIVE_FOLDER_ID):
        ...
        for item in list_dirs:
            ...
            if ospath.isdir(current_file_name):
                current_dir_id = self.__create_directory(item, GDRIVE_FOLDER_ID)
                ...
                self.__upload_file(current_file_name, file_name, mime_type, GDRIVE_FOLDER_ID)
                self.__total_files += 1
                new_id = GDRIVE_FOLDER_ID
            if self.__is_cancelled:
                break
        return new_id
    ...
    def __drive_query(self, GDRIVE_FOLDER_ID, fileName, stopDup, isRecursive, itemType):
        try:
            if isRecursive:
                if stopDup:
                ...
                if stopDup:
                    query = f"'{GDRIVE_FOLDER_ID}' in parents and name = '{fileName}' and "
                else:
                    query = f"'{GDRIVE_FOLDER_ID}' in parents and "
                    fileName = fileName.split()
                    for name in fileName:
                        if name != '':
    ...
    def drive_list(self, fileName, stopDup=False, noMulti=False, isRecursive=True, itemType=""):
        ...
        if len(DRIVES_IDS) > 1:
            ...
            if token_service is not None:
                self.__service = token_service
        for index, GDRIVE_FOLDER_ID in enumerate(DRIVES_IDS):
            isRecur = False if isRecursive and len(GDRIVE_FOLDER_ID) > 23 else isRecursive
            response = self.__drive_query(GDRIVE_FOLDER_ID, fileName, stopDup, isRecur, itemType)
            ...
            for file in response.get('files', []):
                ...
                if mime_type == "application/vnd.google-apps.folder":
                    ...
                    if INDEX_URLS[index] is not None:
                        if isRecur:
                            url_path = "/".join([rquote(n, safe='') for n in self.__get_recursive_list(file, GDRIVE_FOLDER_ID)])
                        else:
                            url_path = rquote(f'{file.get("name")}', safe='')
